...
exports.addToCart = async(req,res)=>{
    //to get product properties rom from request body
    ...
    try{
        ...
        if(product){
            //product already in cart   // quantity incrementing 
            product.quantity+=1
...
exports.incrCartItem = async (req,res)=>{
    ...
    try{
        ...
        if(item){
            ...
            res.status(404).json("Item is not found in the cart")
        }
    }
    catch(error){
        res.status(401).json(error)
